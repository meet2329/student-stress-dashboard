...
export default function DynamicUnivariatePage() {
  ...
  const charts = aiPlan?.univariateCharts || []

  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-8">
      <SectionHeader
        title="Univariate Analysis"
        subtitle={`Single-variable distributions for ${datasetProfile?.analysableColumns?.length || 0} analysable columns`}
      />
      ...
      {isAiFallback && (
        <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-xs text-amber-700 font-medium flex items-center gap-2">
          ...
          Charts selected using deterministic statistical analysis (NVIDIA AI not configured)
        </div>
      )}

      {charts.length === 0 ? (
        <div className="p-8 rounded-2xl bg-slate-50 border border-slate-200 text-center">
          <p className="text-sm text-slate-500 font-medium">Insufficient data for univariate analysis.</p>
          <p className="text-xs text-slate-400 mt-1">The dataset may not contain enough analysable columns.</p>
        </div>
      ) : (
