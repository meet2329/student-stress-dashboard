...
import UploadZone3DEffect from '../3d/UploadZone3DEffect'
...
export default function EmptyEdaState() {
  ...
  return (
    <div className="relative w-full overflow-hidden min-h-[85vh] flex flex-col justify-center py-6">
      {/* Three.js Ambient 3D Data Universe Canvas Backdrop */}
      <DataUniverseBackground variant="light" particleMultiplier={1.1} />

      <motion.div
        initial={{ opacity: 0, y: 15 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.35 }}
        className="relative z-10 max-w-5xl mx-auto py-4 px-4 sm:px-6 space-y-10"
      >
        {/* Hero Header */}
        <div className="text-center space-y-3 max-w-2xl mx-auto">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-blue-50/90 backdrop-blur-sm border border-blue-200/80 text-blue-700 text-xs font-bold shadow-2xs">
        ...
        {/* Upload Zone with 3D Holographic Indicator */}
        <CardTilt3D maxTilt={4} scaleOnHover={1.01} className="max-w-2xl mx-auto">
          <div
            className={`
              relative w-full p-8 sm:p-10 rounded-3xl border-2 border-dashed transition-all cursor-pointer shadow-sm backdrop-blur-xs
              ${dragOver
                ? 'border-blue-500 bg-blue-50/90 scale-[1.01] shadow-blue-500/20 shadow-lg'
                : 'border-slate-300/90 bg-white/90 hover:border-blue-400 hover:bg-slate-50/90 hover:shadow-xl'
              }
            `}
          ...
          >
            ...
            <div className="flex flex-col items-center gap-4 text-center">
              {/* 3D Holographic Upload Crystal */}
              <div className="relative w-16 h-16 rounded-2xl bg-gradient-to-tr from-blue-600 via-indigo-600 to-teal-500 flex items-center justify-center text-white shadow-lg shadow-blue-500/25 overflow-hidden group">
                <div className="absolute inset-0 flex items-center justify-center">
                  <UploadZone3DEffect isDragOver={dragOver} size={54} />
                </div>
              </div>

              <div className="space-y-1.5">
                <p className="text-base font-bold text-slate-800">
                  Drag & drop your CSV dataset here, or <span className="text-blue-600 underline font-extrabold">Browse File</span>
                </p>
        ...
        {/* Instant 1-Click Sample Datasets with 3D Card Tilt */}
            ...
            {SAMPLE_DATASETS.map((sample, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="h-full"
                >
                  <CardTilt3D maxTilt={6} scaleOnHover={1.02} className="h-full">
                    <div className="p-5 rounded-2xl bg-white/95 backdrop-blur-xs border border-slate-200/90 shadow-2xs hover:shadow-lg hover:border-slate-300 transition-all flex flex-col justify-between space-y-4 h-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 pt-2">
          {[
            ...
            <div key={idx} className="flex flex-col items-center text-center gap-1.5 p-4 rounded-2xl bg-white/80 backdrop-blur-xs border border-slate-200/80 shadow-2xs">
